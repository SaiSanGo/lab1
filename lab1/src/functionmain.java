		...
		if(k.find())
		{
			...
			if(command2.contains("!d/d"))
			{
				...
				if(p.contains(val))
				{
					...
					derivative(str, val);
					show(str);
					System.out.println("\nÖ´ÐÐºÄÊ± : "+(System.currentTimeMillis()-a)/1000f+" Ãë ");
				}
				else
					System.out.println("Error, no variable ");
			}
...
public class functionmain {
  ...
  public static void main(String[] args) {
    ...
    if (kk.find()) {
      ...
      System.out.println("Ö´ï¿½Ðºï¿½Ê± : " + (System.currentTimeMillis() - aa) / 1000f + " ï¿½ï¿½ ");
      String command2 = new Scanner(System.in).next();
      str = expression(pp);
      if (command2.contains("!d/d")) {
        String val = command2.substring(4,5);
        if (pp.contains(val)) {
          ...
          derivative(str, val);
          show(str);
          System.out.println("\nÖ´ï¿½Ðºï¿½Ê± : " + (System.currentTimeMillis() - aa) / 1000f + " ï¿½ï¿½ ");
        } else {
          System.out.println("Error, no variable ");
        }
      } else {
